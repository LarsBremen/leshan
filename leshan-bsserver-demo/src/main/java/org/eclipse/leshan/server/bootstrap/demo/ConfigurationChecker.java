...
/**
 * Check a BootstrapConfig is correct. This is a complex process, we need to check if the different
 * objects are in coherence with each other.
 */
public class ConfigurationChecker {
      ...
      switch (sec.securityMode) {
        case NO_SEC:
          ...
          assertIf(!ArrayUtils.isEmpty(sec.publicKeyOrId),
              "NO-SEC mode, public key or ID must be empty");
          assertIf(!ArrayUtils.isEmpty(sec.serverPublicKey),
              "NO-SEC mode, server public key must be empty");
          break;
        case PSK:
          assertIf(ArrayUtils.isEmpty(sec.secretKey),
              "pre-shared-key mode, secret key must not be empty");
          assertIf(ArrayUtils.isEmpty(sec.publicKeyOrId),
              "pre-shared-key mode, public key or id must not be empty");
          break;
        case RPK:
          assertIf(ArrayUtils.isEmpty(sec.secretKey),
              "raw-public-key mode, secret key must not be empty");
  ...
  private static BootstrapConfig.ServerSecurity getSecurityEntry(BootstrapConfig config,
      int shortId) {
