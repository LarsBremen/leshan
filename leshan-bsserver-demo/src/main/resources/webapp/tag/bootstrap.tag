<bootstrap>
    <div class="well well-sm col-md-12" if={serverSecurityInfo.rpk}>
        <h4>The Leshan bootstrap public key
            <small>(SubjectPublicKeyInfo der encoded)</small>
            <button type="button" title="Download server public key(.der)"
                    class="btn btn-default btn-xs" onclick={saveServerPubKey}>
        ...
        <div class="col-md-7">
            <u>Hex : </u>
            <pre>{pkcs8pubkey.hex}</pre>
        </div>
        <div class="col-md-5">
            <u>Base64 : </u>
            <pre>{pkcs8pubkey.base64}</pre>
        </div>
        </p>
    </div>
    <div>
                ...
                <td>
                    <div each={ config.bs }>
                        <p>
                            ...
                            {toHex(security.publicKeyOrId)} </code> </br>
                            Client private key : <code> {toHex(security.secretKey)} </code> </br>
                            Server certificate : <code>
                            {toHex(security.serverPublicKey)} </code> </br>
                            </span>
                        </p>
                    </div>
                </td>
      ...
      function showModal() {
        $.get('api/server/endpoint', function (data) {
          ...
          riot.mount('div#modal', 'bootstrap-modal',
              {server: data, security: {rpk: tag.pkcs8pubkey}});
