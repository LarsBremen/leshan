<x509-input>
    ...
    <div class={ form-group:true, has-error: x509PrivateKey.error }>
        ...
        <div class="col-sm-8">
            <textarea class="form-control" style="resize:none" rows="3" id="x509PrivateKey"
                      ref="x509PrivateKey" oninput={validate_x509PrivateKey}
                      onblur={validate_x509PrivateKey}></textarea>
            <p class="text-right text-muted small" style="margin:0">Hexadecimal format</p>
            <p class="help-block" if={x509PrivateKey.required}>The client private key is
                required</p>
    ...
    <div class={ form-group:true, has-error: x509ServerCert.error }>
        ...
        <div class="col-sm-8">
            <textarea class="form-control" style="resize:none" rows="3" id="x509ServerCert"
                      ref="x509ServerCert" oninput={validate_x509ServerCert}
                      onblur={validate_x509ServerCert}></textarea>
            <p class="text-right text-muted small" style="margin:0">Hexadecimal format</p>
            <p class="help-block" if={x509ServerCert.required}>The server certificate is
                required</p>
    ...
    <script>
      ...
      function has_error() {
        return typeof tag.x509Cert.error === "undefined" || tag.x509Cert.error
            ...
            || typeof tag.x509ServerCert.error === "undefined" || tag.x509ServerCert.error;
      }
      ...
      function get_value() {
        return {
          ...
          servCert: tag.refs.x509ServerCert.value
        };
      }
    </script>
</x509-input>