<bootstrap-modal>
    <div class="modal bs-example-modal-sm" id="bootstrap-modal" tabindex="-1" role="dialog"
         aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form class="form form-horizontal" name="form">
                        <div class={ form-group:true, has-error: endpoint.error }>
                            <label for="endpoint" class="col-sm-4 control-label">Client
                                endpoint</label>
                            <div class="col-sm-8">
                                <input class="form-control" id="endpoint" ref="endpoint"
                                       oninput={validate_endpoint} onblur={validate_endpoint}>
                                <p class="help-block">The endpoint is required</p>
                            </div>
                        </div>

                        <ul class="nav nav-tabs nav-justified">
                            <li role="presentation" class={ active:activetab.lwserver }><a
                                    href="javascript:void(0);" onclick={activetab.activelwserver}>LWM2M
                                Server</a></li>
                            <li role="presentation" class={ active:activetab.bsserver }><a
                                    href="javascript:void(0);" onclick={activetab.activebsserver}>LWM2M
                                Bootstrap Server</a></li>
                        </ul>
                        </br>

                        <div>
                            <securityconfig-input ref="lwserver" onchange={update}
                                                  show={activetab.lwserver}
                                                  securi={
                            "coaps://" + location.hostname + ":5684" }
                            unsecuri= { "coap://" + location.hostname + ":5683" }
                            secmode = { {no_sec:true, psk:true, rpk:true, x509:true} }
                            ></securityconfig-input>
                        </div>
                        <div>
                            <securityconfig-input ref="bsserver" onchange={update}
                                                  show={activetab.bsserver}
                                                  securi={
                            "coaps://" + location.hostname + ":" + serverdata.securedEndpointPort }
                            unsecuri= { "coap://" + location.hostname + ":" +
                            serverdata.unsecuredEndpointPort }
                ...
                <div class="modal-footer">
                    ...
                    <button type="button" class="btn btn-primary" disabled={has_error()}
    ...
    <script>
      ...
      tag.activetab = {
        lwserver: true,
        bsserver: false,
        activelwserver: function () {
          tag.activetab.lwserver = true;
          tag.activetab.bsserver = false;
        },
        activebsserver: function () {
          tag.activetab.lwserver = false;
          tag.activetab.bsserver = true;
        }
      };
