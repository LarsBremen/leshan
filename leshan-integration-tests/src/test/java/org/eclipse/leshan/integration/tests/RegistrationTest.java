...
public class RegistrationTest {
  ...
  public void deregister_cancel_multiple_pending_request() throws InterruptedException {
    ...
    for (int index = 0; index < 4; ++index) {
      ...
      helper.server
          .send(helper.getCurrentRegistration(), new ReadRequest(3, 0, 1), callback, callback);
  ...
  @Test
  public void register_update_deregister_reregister()
  ...
  @Test
  public void register_update_reregister()
      throws NonUniqueSecurityInfoException, InterruptedException {
    // Check client is not registered
    helper.assertClientNotRegisterered();
  ...
  @Test
  public void register_observe_deregister_observe()
      throws NonUniqueSecurityInfoException, InterruptedException {
    // Check client is not registered
    helper.assertClientNotRegisterered();
  ...
  public void fail_to_create_client_with_same_object_twice() {
    ObjectEnabler objectEnabler = new ObjectEnabler(1, null,
        new HashMap<Integer, LwM2mInstanceEnabler>(), null);
    ObjectEnabler objectEnabler2 = new ObjectEnabler(1, null,
        new HashMap<Integer, LwM2mInstanceEnabler>(), null);
  ...
  public void register_with_additional_attributes() throws InterruptedException {
    ...
    assertEquals(additionalAttributes,
        helper.getLastRegistration().getAdditionalRegistrationAttributes());
    assertArrayEquals(Link.parse("</>;rt=\"oma.lwm2m\",</1/0>,</2>,</3/0>,</2000/0>".getBytes()),
        helper.getCurrentRegistration().getObjectLinks());
  }
  ...
  public void register_with_invalid_request() throws InterruptedException, IOException {
    ...
    coapRequest
        .setDestinationContext(new AddressEndpointContext(helper.server.getUnsecuredAddress()));
    ...
    assertEquals(response.getCode(),
        org.eclipse.californium.core.coap.CoAP.ResponseCode.BAD_REQUEST);
    coapEndpoint.stop();
  }
}
