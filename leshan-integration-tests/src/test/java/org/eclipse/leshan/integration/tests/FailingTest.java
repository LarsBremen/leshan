...
public class FailingTest {
  ...
  public void sync_send_without_acknowleged() throws Exception {
    ...
    client.sendLwM2mRequest(
        new RegisterRequest(helper.getCurrentEndpoint(), 60l, null, BindingMode.U, null,
            Link.parse("</1>,</2>,</3>".getBytes()), null));
    client.expectResponse().go();
    helper.waitForRegistrationAtServerSide(1);

    // Send read
    Future<ReadResponse> future = Executors.newSingleThreadExecutor()
        .submit(new Callable<ReadResponse>() {
          @Override
  ...
  public void async_send_without_acknowleged() throws Exception {
    ...
    helper.server
        .send(helper.getCurrentRegistration(), new ReadRequest(3), 3000l, callback, callback);
