...
public class WriteTest {
  ...
  public void cannot_write_security_resource() throws InterruptedException {
    ...
    WriteResponse response = helper.server
        .send(helper.getCurrentRegistration(), new WriteRequest(0, 0, 0, uri));
  ...
  public void can_write_object_instance(ContentFormat format) throws InterruptedException {
    ...
    ReadResponse readResponse = helper.server
  ...
  public void can_write_replacing_object_instance() throws InterruptedException {
    // setup server object
    WriteResponse response = helper.server
        .send(helper.getCurrentRegistration(), new WriteRequest(1, 0, 3, 60));
    ...
    response = helper.server.send(helper.getCurrentRegistration(),
        new WriteRequest(Mode.REPLACE, 1, 0, lifetime, defaultMinPeriod, notificationStoring,
            binding));
    ...
    ReadResponse readResponse = helper.server
        .send(helper.getCurrentRegistration(), new ReadRequest(1, 0));
    LwM2mObjectInstance instance = (LwM2mObjectInstance) readResponse.getContent();
  ...
  public void can_write_multi_instance_objlnk_resource_in_tlv() throws InterruptedException {
    ...
    WriteResponse response = helper.server.send(helper.getCurrentRegistration(),
        new WriteRequest(ContentFormat.TLV, IntegrationTestHelper.TEST_OBJECT_ID, 0,
            IntegrationTestHelper.OBJLNK_MULTI_INSTANCE_RESOURCE_ID, neighbourCellReport,
            Type.OBJLNK));
    ...
    ReadResponse readResponse = helper.server.send(helper.getCurrentRegistration(), new ReadRequest(
        IntegrationTestHelper.TEST_OBJECT_ID, 0,
        IntegrationTestHelper.OBJLNK_MULTI_INSTANCE_RESOURCE_ID));
  ...
  public void send_writerequest_synchronously_with_bad_payload_raises_codeexception()
  ...
  @Test(expected = CodecException.class)
  public void send_writerequest_asynchronously_with_bad_payload_raises_codeexception()
      throws InterruptedException {
    helper.server.send(helper.getCurrentRegistration(),
        new WriteRequest(3, 0, 13, "a string instead of timestamp for currenttime resource"),
