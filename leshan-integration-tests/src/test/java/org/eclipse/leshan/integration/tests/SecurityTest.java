...
public class SecurityTest {
  ...
  public void registered_device_with_psk_to_server_with_psk()
    ...
    helper.getSecurityStore()
        .add(SecurityInfo
            .newPreSharedKeyInfo(helper.getCurrentEndpoint(), GOOD_PSK_ID, GOOD_PSK_KEY));
  ...
  public void dont_sent_request_if_identity_change()
    ...
    helper.getSecurityStore()
        .add(SecurityInfo.newPreSharedKeyInfo(GOOD_ENDPOINT, "anotherPSK", GOOD_PSK_KEY));
    ...
    connector.send(
        RawData.outbound(new byte[0], new AddressEndpointContext(helper.server.getSecuredAddress()),
            callback, false));
    // Wait until new handshake DTLS is done
    EndpointContext endpointContext = callback.getEndpointContext(1000);
    assertEquals(((PreSharedKeyIdentity) endpointContext.getPeerIdentity()).getIdentity(),
        "anotherPSK");
  ...
  @Test
  public void register_update_reregister_device_with_psk_to_server_with_psk()
      throws NonUniqueSecurityInfoException {
    // Create PSK server & start it
    helper.createServer(); // default server support PSK
    helper.server.start();
  ...
  @Test
  public void registered_device_with_bad_psk_identity_to_server_with_psk()
      throws NonUniqueSecurityInfoException {
    // Create PSK server & start it
    helper.createServer(); // default server support PSK
    helper.server.start();
    ...
    helper.getSecurityStore()
        .add(SecurityInfo
            .newPreSharedKeyInfo(helper.getCurrentEndpoint(), BAD_PSK_ID, GOOD_PSK_KEY));
  ...
  @Test
  public void registered_device_with_bad_psk_key_to_server_with_psk()
      throws NonUniqueSecurityInfoException {
    // Create PSK server & start it
    helper.createServer(); // default server support PSK
    helper.server.start();
    ...
    helper.getSecurityStore()
        .add(SecurityInfo
            .newPreSharedKeyInfo(helper.getCurrentEndpoint(), GOOD_PSK_ID, BAD_PSK_KEY));
  ...
  @Test
  public void registered_device_with_psk_and_bad_endpoint_to_server_with_psk()
      throws NonUniqueSecurityInfoException {
    // Create PSK server & start it
    helper.createServer(); // default server support PSK
    helper.server.start();
  ...
  @Test
  public void registered_device_with_bad_rpk_to_server_with_rpk()
      throws NonUniqueSecurityInfoException {
    helper.createServerWithRPK();
    helper.server.start();
  ...
  @Test
  public void registered_device_with_rpk_and_bad_endpoint_to_server_with_rpk()
      throws NonUniqueSecurityInfoException {
    helper.createServerWithRPK();
    helper.server.start();
