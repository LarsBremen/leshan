...
public class DeleteTest {
  ...
  public void delete_created_object_instance() throws InterruptedException {
    // create ACL instance
    helper.server
        .send(helper.getCurrentRegistration(), new CreateRequest(2, new LwM2mObjectInstance(0,
            Arrays.asList(new LwM2mResource[]{LwM2mSingleResource.newIntegerResource(0, 123)}))));

    // try to delete this instance
    DeleteResponse response = helper.server
        .send(helper.getCurrentRegistration(), new DeleteRequest(2, 0));
  ...
  public void cannot_delete_resource() throws InterruptedException {
    ...
    // verify result
    assertEquals(org.eclipse.californium.core.coap.CoAP.ResponseCode.BAD_REQUEST,
        response.getCode());
  }
  ...
  public void cannot_delete_single_manadatory_object_instance() throws InterruptedException {
    // try to create an instance of object 50
    DeleteResponse response = helper.server
        .send(helper.getCurrentRegistration(), new DeleteRequest(3, 0));
  ...
  public void cannot_delete_security_object_instance() throws InterruptedException {
    DeleteResponse response = helper.server
        .send(helper.getCurrentRegistration(), new DeleteRequest(0, 0));
