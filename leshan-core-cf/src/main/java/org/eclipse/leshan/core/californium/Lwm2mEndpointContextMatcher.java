 ...
 *
 * For x.509, only the CN is checked, because the other parts of the distinguished names are removed
 * when converting it into a {@Link Identity}.
 */
public class Lwm2mEndpointContextMatcher extends PrincipalEndpointContextMatcher {
  ...
  protected boolean matchPrincipals(Principal requestedPrincipal, Principal availablePrincipal) {
    if (requestedPrincipal instanceof X500Principal
        || availablePrincipal instanceof X500Principal) {
      try {
