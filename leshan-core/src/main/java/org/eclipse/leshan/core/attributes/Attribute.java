 ...
 *
 * The {@link Attachment} level of the attribute indicates where it can be applied, e.g. the 'pmin'
 * attribute is only applicable to resources, but it can be assigned on all levels and then
 * inherited by underlying resources.
 */
public class Attribute {
  ...
  private static class AttributeModel {
    ...
    private AttributeModel(String coRELinkParam, Attachment attachment,
        Set<AssignationLevel> assignationLevels,
  ...
  static {
    ...
    modelMap.put(MINIMUM_PERIOD,
        new Attribute.AttributeModel(MINIMUM_PERIOD, Attachment.RESOURCE,
            EnumSet
                .of(AssignationLevel.OBJECT, AssignationLevel.INSTANCE, AssignationLevel.RESOURCE),
    ...
    modelMap.put(LESSER_THAN, new AttributeModel(LESSER_THAN, Attachment.RESOURCE,
        EnumSet.of(AssignationLevel.RESOURCE), AccessMode.RW, Double.class));
    modelMap.put(STEP,
        new AttributeModel(STEP, Attachment.RESOURCE, EnumSet.of(AssignationLevel.RESOURCE),
  ...
  public Attribute(String coRELinkParam, Object value) {
    ...
    if (model == null) {
      throw new IllegalArgumentException(
          String.format("Unsupported attribute '%s'", coRELinkParam));
    }
  ...
  /**
   * Ensures that a provided attribute value matches the attribute value type, including trying to
   * perform a correct conversion if the value is a string, e.g.
   *
  ...
  private Object ensureMatchingValue(AttributeModel model, Object value) {
    ...
    if (!expectedClass.equals(value.getClass()) && value instanceof String) {
      ...
      throw new IllegalArgumentException(
          String.format("Attribute '%s' must have a value of type %s",
