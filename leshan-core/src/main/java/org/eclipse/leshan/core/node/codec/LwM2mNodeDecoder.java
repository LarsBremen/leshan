...
public interface LwM2mNodeDecoder {
   ...
   * @throws CodecException if there payload is malformed.
   */
  <T extends LwM2mNode> T decode(byte[] content, ContentFormat format, LwM2mPath path,
      LwM2mModel model,
      Class<T> nodeClass) throws CodecException;

  /**
   * Deserializes a binary content into a list of time-stamped {@link LwM2mNode} ordering by
   * time-stamp.
   *
   * @param content the content
   * @param format the content format
   ...
   * @throws CodecException if there payload is malformed.
   */
  List<TimestampedLwM2mNode> decodeTimestampedData(byte[] content, ContentFormat format,
      LwM2mPath path,
