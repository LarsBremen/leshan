...
public class LwM2mNodeTextDecoder {
  ...
  public static LwM2mNode decode(byte[] content, LwM2mPath path, LwM2mModel model)
    ...
    if (rDesc != null && rDesc.type != null) {
      return LwM2mSingleResource
          .newResource(path.getResourceId(), parseTextValue(strValue, rDesc.type, path),
              rDesc.type);
    } else {
  ...
  private static Object parseTextValue(String value, Type type, LwM2mPath path)
      throws CodecException {
    ...
    switch (type) {
      ...
      case OBJLNK:
        ...
        if (intArr.length != 2) {
          throw new CodecException("Invalid value %s for objectLink resource [%s]",
      ...
      case OPAQUE:
        // not specified
      default:
        throw new CodecException("Could not handle %s value with TEXT encoder for resource %s",
            type, path);
    }
  }
}
