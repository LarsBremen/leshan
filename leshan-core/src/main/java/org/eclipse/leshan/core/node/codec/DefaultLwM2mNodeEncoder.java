...
public class DefaultLwM2mNodeEncoder implements LwM2mNodeEncoder {
  ...
  @Override
  public byte[] encodeTimestampedData(List<TimestampedLwM2mNode> timestampedNodes,
    ...
    LOG.debug("Encoding time-stamped nodes for path {} and format {}", timestampedNodes, path,
        format);

    byte[] encoded;
    switch (format.getCode()) {
      case ContentFormat.JSON_CODE:
        encoded = LwM2mNodeJsonEncoder
            .encodeTimestampedData(timestampedNodes, path, model, converter);
        break;
      default:
        throw new CodecException("Cannot encode timestampedNode with format %s. [%s]", format,
            path);
    }
