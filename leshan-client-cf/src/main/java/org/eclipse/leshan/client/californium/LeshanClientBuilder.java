...
public class LeshanClientBuilder {
  ...
  /**
   * Creates a new instance for setting the configuration options for a {@link LeshanClient}
   * instance.
   *
   * The builder is initialized with the following default values:
   * <ul>
   * <li><em>local address</em>: a local address and an ephemeral port (picked up during
   * binding)</li>
   * <li><em>object enablers</em>:
   * <ul>
   * <li>Security(0) with one instance (DM server security): uri=<em>coap://leshan.eclipse.org:5683</em>,
   * mode=NoSec
   * </li>
   ...
   * </p>
   * Warning : The Security ObjectEnabler should not contains 2 or more entries with the same
   * identity. This is not a LWM2M specification constraint but an implementation limitation.
   */
  ...
  public LeshanClient build() {
    ...
    if (objectEnablers == null) {
      ...
      initializer
          .setInstancesForObject(LwM2mId.SERVER, new Server(12345, 5 * 60, BindingMode.U, false));
      initializer.setInstancesForObject(LwM2mId.DEVICE,
          new Device("Eclipse Leshan", "model12345", "12345", "U"));
    ...
    return new LeshanClient(endpoint, localAddress, objectEnablers, coapConfig, dtlsConfigBuilder,
        endpointFactory,
