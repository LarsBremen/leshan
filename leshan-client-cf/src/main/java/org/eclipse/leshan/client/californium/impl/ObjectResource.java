...
public class ObjectResource extends CoapResource implements NotifySender {
  ...
  public void handleRequest(Exchange exchange) {
    try {
      ...
      if (LOG.isDebugEnabled()) {
        LOG.debug(
            String.format("InvalidRequestException while handling request(%s) on the %s resource",
      ...
      LOG.error(String
          .format("Exception while handling request(%s) on the %s resource", exchange.getRequest(),
              getURI()), e);
    ...
    if (exchange.getRequestOptions().getAccept() == MediaTypeRegistry.APPLICATION_LINK_FORMAT) {
      ...
      if (response.getCode().isError()) {
        ...
        exchange.respond(toCoapResponseCode(response.getCode()),
            Link.serialize(response.getObjectLinks()),
  ...
  public void handlePUT(CoapExchange coapExchange) {
    ...
    // Manage Write and Bootstrap Write Request (replace)
      ...
      ContentFormat contentFormat = ContentFormat
          .fromCode(coapExchange.getRequestOptions().getContentFormat());
      ...
      try {
        ...
        if (identity.isLwm2mBootstrapServer()) {
          ...
          if (response.getCode().isError()) {
            coapExchange
                .respond(toCoapResponseCode(response.getCode()), response.getErrorMessage());
          } else {
            coapExchange.respond(toCoapResponseCode(response.getCode()));
          }
        } else {
  ...
  public void handlePOST(CoapExchange exchange) {
    ...
    ContentFormat contentFormat = ContentFormat
        .fromCode(exchange.getRequestOptions().getContentFormat());
    if (!decoder.isSupported(contentFormat)) {
    ...
    if (path.isObjectInstance()) {
      try {
        LwM2mNode lwM2mNode = decoder
            .decode(exchange.getRequestPayload(), contentFormat, path, model);
        WriteResponse response = nodeEnabler.write(identity,
