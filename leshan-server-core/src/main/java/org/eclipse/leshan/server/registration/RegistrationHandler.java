...
/**
 * Handle the client registration logic. Check if the client is allowed to register, with the wanted
 * security scheme. Create the {@link Registration} representing the registered client and add it to
 * the {@link RegistrationService}
 */
public class RegistrationHandler {
  ...
  public SendableResponse<RegisterResponse> register(Identity sender,
      RegisterRequest registerRequest,
      InetSocketAddress serverEndpoint) {

    Registration.Builder builder = new Registration.Builder(
        registrationIdProvider.getRegistrationId(registerRequest),
        registerRequest.getEndpointName(), sender,
        serverEndpoint);

    builder.lwM2mVersion(registerRequest.getLwVersion())
        ...
        .smsNumber(registerRequest.getSmsNumber()).registrationDate(new Date())
        .lastUpdate(new Date())
    ...
    final Registration registration = authorizer
        .isAuthorized(registerRequest, builder.build(), sender);
  ...
  public SendableResponse<UpdateResponse> update(Identity sender, UpdateRequest updateRequest) {
    ...
    final RegistrationUpdate update = new RegistrationUpdate(updateRequest.getRegistrationId(),
        sender,
        updateRequest.getLifeTimeInSec(), updateRequest.getSmsNumber(),
        updateRequest.getBindingMode(),
        updateRequest.getObjectLinks(), updateRequest.getAdditionalAttributes());

    // update registration
    final UpdatedRegistration updatedRegistration = registrationService.getStore()
  ...
  public SendableResponse<DeregisterResponse> deregister(Identity sender,
