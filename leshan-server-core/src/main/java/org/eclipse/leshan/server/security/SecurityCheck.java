...
public class SecurityCheck {
  ...
  public static boolean checkSecurityInfos(String endpoint, Identity clientIdentity,
    ...
    if (clientIdentity.isSecure()) {
      if (securityInfos == null || securityInfos.isEmpty()) {
        LOG.debug("Client '{}' without security info try to connect through the secure endpoint",
            endpoint);
        return false;
      } else {
   ...
   */
  public static boolean checkSecurityInfo(String endpoint, Identity clientIdentity,
  ...
  private static boolean checkPskIdentity(String endpoint, Identity clientIdentity,
      SecurityInfo securityInfo) {
  ...
  private static boolean checkRpkIdentity(String endpoint, Identity clientIdentity,
      SecurityInfo securityInfo) {
  ...
  private static boolean checkX509Identity(String endpoint, Identity clientIdentity,
      SecurityInfo securityInfo) {
