 ...
 *
 * It checks in {@link SecurityStore} if there is a corresponding {@link SecurityInfo} for this
 * registration endpoint. If there is a {@link SecurityInfo} it check the identity is correct, else
 * it checks if the LWM2M client use an unsecure connection.
 */
public class DefaultAuthorizer implements Authorizer {
  ...
  public Registration isAuthorized(UplinkRequest<?> request, Registration registration,
    ...
    if (SecurityCheck
        .checkSecurityInfo(registration.getEndpoint(), senderIdentity, expectedSecurityInfo)) {
      return registration;
    } else {
      return null;
    }
