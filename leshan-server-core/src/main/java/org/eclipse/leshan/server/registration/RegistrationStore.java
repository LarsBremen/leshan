...
/**
 * A store for registrations and observations. This interface is also responsible to handle
...
public interface RegistrationStore {
   ...
   *
   * If a registration already exists with the given endpoint, the store is in charge of removing
   * this registration as well as the ongoing observations.
   *
   * @param registration the new registration.
   * @return the old registration and its observations or <code>null</code> if it does not already
   * exists.
   */
  ...
  /**
   * Get the registration by socket address. If there are 2 Registrations linked to the same
   * address, the most recent one should be returned. Generally this happened when devices are
   * behind NAT and so address could be reused.
   *
  ...
  /**
   * Returns an iterator over the registration of this store. There are no guarantees concerning the
   * order in which the elements are returned (unless the implementation provides a guarantee).
   *
   ...
   * @return the registration and observations removed or null if there is no registration for this
   * Id.
   */
   ...
   *
   * The store is in charge of removing the observations already existing for the same path and
   * registration id.
   *
