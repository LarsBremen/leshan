...
public final class PresenceServiceImpl implements PresenceService {
  ...
  /**
   * Notify the listeners that the client state changed to {@link Presence#SLEEPING}. The state
   * changes is produced inside {@link PresenceStatus} when the timer expires or when the client
   * doesn't respond to a request.
   *
   * @param reg the client's registration object
   */
  ...
  /**
   * Start or restart (if already started) the timer that handles the client wait before sleep
   * time.
   */
