 ...
 *
 * Will receive client registration through the "/rd" resource. Is able to send requests (Read,
 * Write, Create, Delete, Execute, Discover, Observer) to specified clients.
 *
...
public interface LwM2mServer {
  ...
  /**
   * Stops the server, i.e. unbinds it from all ports. Frees as much system resources as possible to
   * still be able to be started.
   */
  void stop();
  ...
  /**
   * Sends a Lightweight M2M request synchronously. Will block until a response is received from the
   * remote client.
   *
   ...
   * This exception will never be raised if Queue Mode support is deactivate.
   */
  ...
  /**
   * Get the registration service to access to registered clients. You can use this object for
   * listening client registration lifecycle.
   */
  RegistrationService getRegistrationService();

  /**
   * Get the Observation service to access current observations. You can use this object for
   * listening resource observation or cancel it.
   */
  ObservationService getObservationService();

  /**
   * Get the Presence service to get the status of LWM2M clients connected with binding mode 'Q'.
   * You can use this object to add {@link PresenceListener} to get notified when a device comes
   * online or offline.
   */
