...
public class SecurityServlet extends HttpServlet {
  ...
  protected void doPut(HttpServletRequest req, HttpServletResponse resp)
    ...
    try {
      SecurityInfo info = gsonDes
          .fromJson(new InputStreamReader(req.getInputStream()), SecurityInfo.class);
  ...
  protected void doGet(HttpServletRequest req, HttpServletResponse resp)
    ...
    if ("server".equals(path[0])) {
      String json = this.gsonSer
          .toJson(SecurityInfo.newRawPublicKeyInfo("leshan", serverPublicKey));
      resp.setContentType("application/json");
      resp.getOutputStream().write(json.getBytes(StandardCharsets.UTF_8));
      resp.setStatus(HttpServletResponse.SC_OK);
      return;
