...
.directive('resource',
    function ($compile, $routeParams, $http, dialog, $filter, lwResources,
        helper) {
      return {
        restrict: "E",
        replace: true,
        ...
        link: function (scope, element, attrs) {
          ...
          scope.resource.exec = {
            tooltip: "Execute <br/>" + scope.resource.path
          };
          scope.resource.execwithparams = {
            tooltip: "Execute with parameters<br/>" + scope.resource.path
          };
          scope.resource.observe = {
            tooltip: "Observe <br/>" + scope.resource.path
          };
          ...
          scope.startObserve = function () {
            ...
            var uri = "api/clients/" + $routeParams.clientId
                + scope.resource.path + "/observe";
            $http.post(uri, null, {params: {format: format}})
            .success(function (data, status, headers, config) {
              helper.handleResponse(data, scope.resource.observe,
              ...
              errormessage = "Unable to start observation on resource "
                  + scope.resource.path + " for " + $routeParams.clientId
                  + " : " + status + " " + data;
          ...
          scope.stopObserve = function () {
            ...
            .success(function (data, status, headers, config) {
              ...
              errormessage = "Unable to stop observation on resource "
                  + scope.resource.path + " for " + $routeParams.clientId
                  + " : " + status + " " + data;
              dialog.open(errormessage);
              console.error(errormessage);
          ...
          scope.read = function () {
            ...
            .success(function (data, status, headers, config) {
              ...
              helper.handleResponse(data, scope.resource.read,
                  function (formattedDate) {
              ...
              errormessage = "Unable to read resource " + scope.resource.path
                  + " for " + $routeParams.clientId + " : " + status + " "
                  + data;
              dialog.open(errormessage);
              console.error(errormessage);
            });
          ...
          scope.write = function () {
            ...
            $('#writeSubmit').click(function (e) {
              ...
              if (value != undefined) {
                ...
                value = lwResources.getTypedValue(value,
                    scope.resource.def.type);
                rsc["value"] = value;

                var format = scope.settings.single.format;
                $http({
                  method: 'PUT',
                  url: "api/clients/" + $routeParams.clientId
                      + scope.resource.path,
                  data: rsc,
                ...
                .success(function (data, status, headers, config) {
                  helper.handleResponse(data, scope.resource.write,
                      function (formattedDate) {
                        if (data.success) {
                  ...
                  errormessage = "Unable to write resource "
                      + scope.resource.path + " for " + $routeParams.clientId
                      + " : " + status + " " + data;
                  dialog.open(errormessage);
                  console.error(errormessage);
          ...
          scope.exec = function () {
            $http.post(
                "api/clients/" + $routeParams.clientId + scope.resource.path)
            .success(function (data, status, headers, config) {
              helper.handleResponse(data, scope.resource.exec);
            }).error(function (data, status, headers, config) {
              errormessage = "Unable to execute resource " + scope.resource.path
                  + " for " + $routeParams.clientId + " : " + status + " "
                  + data;
              dialog.open(errormessage);
              console.error(errormessage);
            ...
            $('#writeSubmit').click(function (e) {
              ...
              if (value) {
                ...
                .success(function (data, status, headers, config) {
                  ...
                  errormessage = "Unable to execute resource "
                      + scope.resource.path + " for " + $routeParams.clientId
                      + " : " + status + " " + data;
                  dialog.open(errormessage);
                  console.error(errormessage);
