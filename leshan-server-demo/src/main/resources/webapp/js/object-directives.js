...
.directive('object',
    function ($compile, $routeParams, $http, dialog, $filter, $modal,
        lwResources, helper) {
      return {
        restrict: "E",
        replace: true,
        ...
        link: function (scope, element, attrs) {
          ...
          scope.create = function () {
            ...
            modalInstance.result.then(function (instance) {
              ...
              $http({
                method: 'POST',
                url: "api/clients/" + $routeParams.clientId + instancepath,
                data: payload,
                headers: {'Content-Type': 'application/json'},
                params: {format: format}
              })
              .success(function (data, status, headers, config) {
                helper.handleResponse(data, scope.object.create,
                    function (formattedDate) {
                      if (data.success) {
                        var newinstance = lwResources.addInstance(scope.object,
                            instance.id, null);
                        for (var i in payload.resources) {
                          var tlvresource = payload.resources[i];
                          resource = lwResources.addResource(scope.object,
                              newinstance, tlvresource.id, null);
                ...
                errormessage = "Unable to create instance " + instancepath
