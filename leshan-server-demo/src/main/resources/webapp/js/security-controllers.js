...
.controller('SecurityCtrl', [
  ...
  function SecurityCtrl($scope, $http, dialog) {
    ...
    $http.get('api/security/clients').error(
        function (data, status, headers, config) {
          $scope.error = "Unable to get the clients security info list: "
              + status + " " + data;
    ...
    $http.get('api/security/server').error(
        function (data, status, headers, config) {
          $scope.error = "Unable to get the server security info list: "
              + status + " " + data;
          console.error($scope.error);
        }).success(function (data, status, headers, config) {
    ...
    $scope.remove = function (endpoint) {
      $http({
        method: 'DELETE',
        url: "api/security/clients/" + endpoint,
        headers: {'Content-Type': 'text/plain'}
      })
      .success(function (data, status, headers, config) {
        delete $scope.securityInfos[endpoint];
      }).error(function (data, status, headers, config) {
        errormessage = "Unable to remove security info for endpoint " + endpoint
            + ": " + status + " - " + data;
    ...
    $scope.saveServerPubKey = function (serverPubKey) {
      var blob = new Blob($scope.pkcs8pubkey.bytes,
      ...
      if ($scope.form.$valid) {
        if ($scope.securityMode == "psk") {
          var security = {
            ...
            psk: {identity: $scope.pskIdentity, key: $scope.pskValue}
          };
          ...
          var security = {
            ...
            rpk: {
              x: $scope.rpkXValue,
              y: $scope.rpkYValue,
              params: $scope.rpkParamsValue || $scope.defaultParams
            }
          };
        } else {
        ...
        if (security) {
          ...
          .success(function (data, status, headers, config) {
            ...
            errormessage = "Unable to add security info for endpoint "
                + $scope.endpoint + ": " + status + " - " + data;
            dialog.open(errormessage);
            console.error(errormessage);
          });
        }
