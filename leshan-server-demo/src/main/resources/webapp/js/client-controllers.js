...
lwClientControllers.controller('ClientListCtrl', [
  ...
  function ClientListCtrl($scope, $http, $location) {
    ...
    $http.get('api/clients').error(function (data, status, headers, config) {
      ...
      $scope.eventsource.addEventListener('REGISTRATION', registerCallback,
          false);
      ...
      $scope.eventsource.addEventListener('DEREGISTRATION', deregisterCallback,
          false);
    });
  }]);
...
lwClientControllers.controller('ClientDetailCtrl', [
  ...
  function ($scope, $location, $routeParams, $http, lwResources, $filter) {
    ...
    .error(function (data, status, headers, config) {
      $scope.error = "Unable to get client " + $routeParams.clientId + " : "
          + status + " " + data;
      console.error($scope.error);
    })
    .success(function (data, status, headers, config) {
      ...
      lwResources.buildResourceTree($scope.client.rootPath,
          $scope.client.objectLinks, function (objects) {
      ...
      var notificationCallback = function (msg) {
        $scope.$apply(function () {
          ...
          if (resource) {
            ...
            // instance?
            var instance = lwResources.findInstance($scope.objects,
                content.res);
            if (instance) {
              instance.observed = true;
              for (var i in content.val.resources) {
                var tlvresource = content.val.resources[i];
                resource = lwResources.addResource(instance.parent, instance,
                    tlvresource.id, null);
      ...
      $scope.eventsource.addEventListener('NOTIFICATION', notificationCallback,
          false);
      ...
      var coapLogCallback = function (msg) {
        $scope.$apply(function () {
          ...
          if (256 < $scope.coaplogs.length) {
            $scope.coaplogs.shift();
          }
