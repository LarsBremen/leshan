    ...
    function ($compile, $routeParams, $http, dialog, $filter, lwResources,
      ...
      return {
        ...
        link: function (scope, element, attrs) {
          ...
          scope.instance.observe = {
            tooltip: "Observe <br/>" + scope.instance.path
          };
          ...
          scope.read = function () {
            ...
            .success(function (data, status, headers, config) {
              helper.handleResponse(data, scope.instance.read,
                  function (formattedDate) {
                    if (data.success && data.content) {
                      for (var i in data.content.resources) {
                        var tlvresource = data.content.resources[i];
                        resource = lwResources.addResource(scope.parent,
                            scope.instance, tlvresource.id, null);
              ...
              errormessage = "Unable to read instance " + scope.instance.path
          ...
          scope.del = function () {
            ...
            $http.delete(uri)
            .success(function (data, status, headers, config) {
              helper.handleResponse(data, scope.instance.del,
                  function (formattedDate) {
              ...
              errormessage = "Unable to delete instance " + scope.instance.path
                  + " for " + $routeParams.clientId + " : " + status + " "
                  + data;
              dialog.open(errormessage);
              console.error(errormessage);
          ...
          scope.write = function () {
            ...
            modalInstance.result.then(function (instance) {
              ...
              for (i in instance.resources) {
                ...
                if (resource.value != undefined) {
                  payload.resources.push({
                    id: resource.id,
                    value: lwResources.getTypedValue(resource.value,
                        resource.def.type)
                  });
                }
              }
              ...
              .success(function (data, status, headers, config) {
                helper.handleResponse(data, scope.instance.write,
                    function (formattedDate) {
                      if (data.success) {
                ...
                errormessage = "Unable to write resource " + scope.instance.path
                    + " for " + $routeParams.clientId + " : " + status + " "
                    + data;
                dialog.open(errormessage);
                console.error(errormessage);
          ...
          scope.startObserve = function () {
            var format = scope.settings.multi.format;
            var uri = "api/clients/" + $routeParams.clientId
                + scope.instance.path + "/observe";
            $http.post(uri, null, {params: {format: format}})
            .success(function (data, status, headers, config) {
              helper.handleResponse(data, scope.instance.observe,
                  function (formattedDate) {
                    if (data.success) {
              ...
              errormessage = "Unable to start observation on instance "
                  + scope.instance.path + " for " + $routeParams.clientId
                  + " : " + status + " " + data;
              dialog.open(errormessage);
              console.error(errormessage);
            });
          ...
          scope.stopObserve = function () {
            ...
            .success(function (data, status, headers, config) {
              ...
              errormessage = "Unable to stop observation on instance "
                  + scope.instance.path + " for " + $routeParams.clientId
                  + " : " + status + " " + data;
              dialog.open(errormessage);
              console.error(errormessage);
