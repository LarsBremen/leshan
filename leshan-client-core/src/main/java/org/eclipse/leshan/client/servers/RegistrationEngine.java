 ...
 * <li>If communication failed with device management server, try to bootstrap again each 10 minutes
 * until succeed</li>
 * </ul>
 * <br>
...
public class RegistrationEngine {
  ...
  public RegistrationEngine(String endpoint, Map<Integer, LwM2mObjectEnabler> objectEnablers,
      EndpointsManager endpointsManager, LwM2mRequestSender requestSender,
      BootstrapHandler bootstrapState,
  ...
  public Collection<Server> factoryBootstrap() {
    ...
    if (!serversInfo.deviceMangements.isEmpty()) {
      Collection<Server> servers = endpointsManager
          .createEndpoints(serversInfo.deviceMangements.values());
      return servers;
    }
    return null;
  }
  ...
  private Collection<Server> clientInitiatedBootstrap() throws InterruptedException {
    ...
    if (bootstrapHandler.tryToInitSession(bootstrapServerInfo)) {
      ...
      try {
        BootstrapResponse response = sender
            .send(bootstrapServer.getPeerAddress(), bootstrapServer.isSecure(),
        ...
        if (response == null) {
          ...
          if (observer != null) {
            observer.onBootstrapFailure(bootstrapServer, response.getCode(),
  ...
  private boolean register(Server server) throws InterruptedException {
    ...
    try {
      RegisterRequest regRequest = new RegisterRequest(endpoint, dmInfo.lifetime, LwM2m.VERSION,
          dmInfo.binding,
          null, LinkFormatHelper.getClientDescription(objectEnablers.values(), null),
  ...
  private boolean deregister(Server server, String registrationID) throws InterruptedException {
    ...
    try {
      DeregisterResponse response = sender.send(server.getIdentity().getPeerAddress(),
          server.getIdentity().isSecure(), new DeregisterRequest(registrationID),
      ...
      if (response == null) {
        ...
        if (observer != null) {
          if (response.isSuccess()) {
            ...
            observer
                .onDeregistrationFailure(server, response.getCode(), response.getErrorMessage());
          }
        }
        return true;
      } else {
  ...
  private boolean update(Server server, String registrationID) throws InterruptedException {
    ...
    try {
      ...
      if (response == null) {
        ...
        LOG.error("Registration update failed: {} {}.", response.getCode(),
            response.getErrorMessage());
        if (observer != null) {
  ...
  private synchronized void scheduleRegistrationTask(Server dmServer, long timeInMs) {
    ...
    if (timeInMs > 0) {
      ...
      registerFuture = schedExecutor
          .schedule(new RegistrationTask(dmServer), timeInMs, TimeUnit.MILLISECONDS);
    } else {
      registerFuture = schedExecutor.submit(new RegistrationTask(dmServer));
    }
    return;
  ...
  private synchronized void scheduleUpdate(Server server, String registrationId, long timeInMs) {
    ...
    if (timeInMs > 0) {
      LOG.info("Next registration update to {} in {}s...", server.getIdentity().getPeerAddress(),
          timeInMs / 1000);
      updateFuture = schedExecutor
          .schedule(new UpdateRegistrationTask(server, registrationId), timeInMs,
              TimeUnit.MILLISECONDS);
    } else {
