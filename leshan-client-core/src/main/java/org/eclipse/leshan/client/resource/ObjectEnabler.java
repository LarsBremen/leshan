...
public class ObjectEnabler extends BaseObjectEnabler {
  ...
  public ObjectEnabler(int id, ObjectModel objectModel,
  ...
  protected ReadResponse doRead(ServerIdentity identity, ReadRequest request) {
    ...
    if (path.isObject()) {
      ...
      for (Entry<Integer, LwM2mInstanceEnabler> entry : instances.entrySet()) {
        lwM2mObjectInstances
            .add(getLwM2mObjectInstance(entry.getKey(), entry.getValue(), identity, false));
      }
    ...
    if (path.getResourceId() == null) {
      return ReadResponse
          .success(getLwM2mObjectInstance(path.getObjectInstanceId(), instance, identity, false));
    }
  ...
  protected ObserveResponse doObserve(final ServerIdentity identity, final ObserveRequest request) {
    ...
    if (path.isObject()) {
      ...
      for (Entry<Integer, LwM2mInstanceEnabler> entry : instances.entrySet()) {
        lwM2mObjectInstances
            .add(getLwM2mObjectInstance(entry.getKey(), entry.getValue(), identity, true));
      }
    ...
    for (ResourceModel resourceModel : getObjectModel().resources.values()) {
      ...
      if (identity.isSystem() || resourceModel.operations.isReadable()) {
        if (observe) {
          ...
          if (response.getCode() == ResponseCode.CONTENT && response
              .getContent() instanceof LwM2mResource) {
  ...
  protected WriteResponse doWrite(ServerIdentity identity, WriteRequest request) {
    ...
    if (path.isObjectInstance()) {
      // instance write
      Map<Integer, LwM2mResource> writeResources = ((LwM2mObjectInstance) request.getNode())
          .getResources();
  ...
  protected BootstrapWriteResponse doWrite(ServerIdentity identity, BootstrapWriteRequest request) {
    ...
    // Manage Object case
    if (path.isObject()) {
      for (LwM2mObjectInstance instanceNode : ((LwM2mObject) request.getNode()).getInstances()
          .values()) {
        ...
        if (instanceEnabler == null) {
          ...
          doWrite(identity,
              new WriteRequest(Mode.REPLACE, path.getObjectId(), path.getObjectInstanceId(),
    ...
    if (path.isObjectInstance()) {
      ...
      if (instanceEnabler == null) {
        ...
        doWrite(identity,
            new WriteRequest(Mode.REPLACE, request.getContentFormat(), path.getObjectId(),
