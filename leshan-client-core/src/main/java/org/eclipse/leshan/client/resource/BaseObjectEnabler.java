...
public abstract class BaseObjectEnabler implements LwM2mObjectEnabler {
  ...
  public synchronized WriteResponse write(ServerIdentity identity, WriteRequest request) {
    ...
    if (path.isResource()) {
      ...
      if (LwM2mId.SECURITY != id) { // security resources are writable by SYSTEM
        ...
        for (Integer writeResourceId : ((LwM2mObjectInstance) request.getNode()).getResources()
            .keySet()) {
      ...
      if (request.isReplaceRequest()) {
        ...
        for (ResourceModel resourceModel : getObjectModel().resources.values()) {
          if (resourceModel.mandatory && (LwM2mId.SECURITY == id || resourceModel.operations
              .isWritable())) {
  ...
  @Override
  public synchronized BootstrapWriteResponse write(ServerIdentity identity,
      BootstrapWriteRequest request) {

    // We should not get a bootstrapWriteRequest from a LWM2M server
    if (!identity.isLwm2mBootstrapServer()) {
      return BootstrapWriteResponse
          .internalServerError("bootstrap write request from LWM2M server");
    }
  ...
  protected DiscoverResponse doDiscover(DiscoverRequest request) {
    ...
    if (path.isObject()) {
      ...
      Link[] instanceLink = LinkFormatHelper
          .getInstanceDescription(this, path.getObjectInstanceId(), null);
