...
public class CoapRequestBuilderTest {
  ...
  public void build_read_request() throws Exception {
    ...
    // test
    CoapRequestBuilder builder = new CoapRequestBuilder(reg.getIdentity(), reg.getRootPath(),
        reg.getId(),
    ...
    assertEquals("127.0.0.1",
        coapRequest.getDestinationContext().getPeerAddress().getAddress().getHostAddress());
  ...
  public void build_write_request() throws Exception {
    ...
    CoapRequestBuilder builder = new CoapRequestBuilder(reg.getIdentity(), reg.getRootPath(),
        reg.getId(),
        reg.getEndpoint(), model, encoder);
    WriteRequest request = new WriteRequest(Mode.UPDATE, 3, 0,
        LwM2mSingleResource.newStringResource(4, "value"));
    builder.visit(request);
