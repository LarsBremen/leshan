 ...
 * <p>
 * This resource is the entry point of the Resource Directory ("/rd"). Each new client is added to
...
public class RegisterResource extends CoapResource {
  ...
  public void handleRequest(Exchange exchange) {
    try {
      ...
      LOG.debug("InvalidRequestException while handling request({}) on the /rd resource",
          exchange.getRequest(),
          e);
      ...
      LOG.error("Exception while handling request({}) on the /rd resource", exchange.getRequest(),
          e);
  ...
  private void handleRegister(CoapExchange exchange, Request request) {
    ...
    // Create request
    RegisterRequest registerRequest = new RegisterRequest(endpoint, lifetime, lwVersion, binding,
        smsNumber,
  ...
  private void handleUpdate(CoapExchange exchange, Request request, String registrationId) {
    ...
    UpdateRequest updateRequest = new UpdateRequest(registrationId, lifetime, smsNumber, binding,
        objectLinks,
        additionalParams);

    // Handle request
    final SendableResponse<UpdateResponse> sendableResponse = registrationHandler
        .update(sender, updateRequest);
    UpdateResponse updateResponse = sendableResponse.getResponse();

    // Create CoAP Response from LwM2m request
    if (updateResponse.getCode().isError()) {
      exchange
          .respond(toCoapResponseCode(updateResponse.getCode()), updateResponse.getErrorMessage());
    } else {
  ...
  private void handleDeregister(CoapExchange exchange, String registrationId) {
    ...
    // Handle request
    final SendableResponse<DeregisterResponse> sendableResponse = registrationHandler
        .deregister(sender,
            deregisterRequest);
    ...
    if (deregisterResponse.getCode().isError()) {
      exchange.respond(toCoapResponseCode(deregisterResponse.getCode()),
          deregisterResponse.getErrorMessage());
    } else {
      exchange.respond(toCoapResponseCode(deregisterResponse.getCode()));
    }
    sendableResponse.sent();
