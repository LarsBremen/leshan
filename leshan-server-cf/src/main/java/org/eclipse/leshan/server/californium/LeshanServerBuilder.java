 ...
 * Usage: create it, call the different setters for changing the configuration and then call the
 * {@link #build()} method for creating the {@link LwM2mServer} ready to operate.
 */
public class LeshanServerBuilder {
   ...
   * @param port A valid port value is between 0 and 65535. A port number of zero will let the
  ...
  /**
   * <p>
   * Set your {@link RegistrationStore} implementation which stores {@link Registration} and {@link
   * Observation}.
   * </p>
   ...
   * </p>
   * By default no security store is set. It is needed for secured connection if you are using the
   * defaultAuthorizer or if you want PSK feature activated. An {@link InMemorySecurityStore} is
   * provided to start using secured connection.
   */
  ...
  /**
   * <p>
   * Set your {@link Authorizer} implementation to define if a device if authorize to register to
   * this server.
   * </p>
   * By default the {@link DefaultAuthorizer} implementation is used, it needs a security store to
   * accept secured connection.
   */
   ...
   * </p>
   * By default the {@link StandardModelProvider} implementation is used which support all core
  ...
  /**
   * <p>
   * Set the {@link PublicKey} of the server which will be used for RawPublicKey DTLS
   * authentication.
   * </p>
   * This should be used for RPK support only. If you support RPK and X509, {@link
   * LeshanServerBuilder#setCertificateChain(X509Certificate[])} should be used.
   */
  ...
  /**
   * Set the {@link PrivateKey} of the server which will be used for RawPublicKey(RPK) and X509 DTLS
   * authentication.
   */
   ...
   * </p>
   * For RPK the public key will be extract from the first X509 certificate of the certificate
   * chain. If you only need RPK support, use {@link LeshanServerBuilder#setPublicKey(PublicKey)}
   * instead.
   */
   ...
   */
  public <T extends Certificate> LeshanServerBuilder setTrustedCertificates(
  ...
  /**
   * <p>
   * Set the {@link LwM2mNodeEncoder} which will encode {@link LwM2mNode} with supported content
   * format.
   * </p>
   * By default the {@link DefaultLwM2mNodeEncoder} is used. It supports Text, Opaque, TLV and JSON
   * format.
   */
  ...
  /**
   * <p>
   * Set the {@link LwM2mNodeDecoder} which will decode data in supported content format to create
   * {@link LwM2mNode}.
   * </p>
   * By default the {@link DefaultLwM2mNodeDecoder} is used. It supports Text, Opaque, TLV and JSON
   * format.
   */
  ...
  /**
   * Used to create custom CoAP endpoint, this is only for advanced users. <br> DTLSConnector is
   * expected for secured endpoint.
   */
  ...
  /**
   * deactivate PresenceService which tracks presence of devices using LWM2M Queue Mode. When Queue
   * Mode is deactivated request is always sent immediately and {@link ClientSleepingException} will
   * never be raised. Deactivate QueueMode can make sense if you want to handle it on your own or if
   * you don't plan to support devices with queue mode.
   */
  ...
  /**
   * Sets a new {@link RegistrationIdProvider} object different from the default one (Random
   * string).
   *
  ...
  public LeshanServer build() {
    ...
    if (!noSecuredEndpoint) {
      ...
      } else if (localSecureAddress != null && !localSecureAddress
          .equals(incompleteConfig.getAddress())) {
      ...
      if (incompleteConfig.getStaleConnectionThreshold() == null) {
        dtlsConfigBuilder
            .setStaleConnectionThreshold(coapConfig.getLong(Keys.MAX_PEER_INACTIVITY_PERIOD));
      }
      ...
      if (privateKey != null) {
        if (incompleteConfig.getPrivateKey() != null && !incompleteConfig.getPrivateKey()
            .equals(privateKey)) {
          throw new IllegalStateException(String.format(
        ...
        if (certificateChain == null && publicKey != null) {
          if (incompleteConfig.getPublicKey() != null && !incompleteConfig.getPublicKey()
              .equals(publicKey)) {
            throw new IllegalStateException(String.format(
    ...
    if (!noSecuredEndpoint && dtlsConfig != null) {
      if (endpointFactory != null) {
        securedEndpoint = endpointFactory
            .createSecuredEndpoint(dtlsConfig, coapConfig, registrationStore);
      } else {
        CoapEndpoint.CoapEndpointBuilder builder = new CoapEndpoint.CoapEndpointBuilder();
