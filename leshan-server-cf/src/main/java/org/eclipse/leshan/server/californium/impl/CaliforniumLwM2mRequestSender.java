...
public class CaliforniumLwM2mRequestSender implements LwM2mRequestSender, CoapRequestSender {
   ...
   */
  public CaliforniumLwM2mRequestSender(Set<Endpoint> endpoints,
  ...
  @Override
  public <T extends LwM2mResponse> T send(final Registration destination,
    ...
    SyncRequestObserver<T> syncMessageObserver = new SyncRequestObserver<T>(coapRequest, timeout) {
      ...
      public T buildResponse(Response coapResponse) {
        ...
        LwM2mResponseBuilder<T> lwm2mResponseBuilder = new LwM2mResponseBuilder<>(coapRequest,
            coapResponse,
  ...
  public <T extends LwM2mResponse> void send(final Registration destination,
    ...
    MessageObserver obs = new AsyncRequestObserver<T>(coapRequest, responseCallback, errorCallback,
        timeout) {
      @Override
      public T buildResponse(Response coapResponse) {
        // Build LwM2m response
  ...
  @Override
  public Response sendCoapRequest(final Registration destination, final Request coapRequest,
      long timeout)
  ...
  @Override
  public void sendCoapRequest(final Registration destination, final Request coapRequest,
      long timeout,
    ...
    // Add CoAP request callback
    MessageObserver obs = new CoapAsyncRequestObserver(coapRequest, responseCallback, errorCallback,
        timeout);
    coapRequest.addMessageObserver(obs);
   ...
   * @return the CoAP endpoint bound to the same network address and port that the client connected
   * to during registration. If no such CoAP endpoint is available, the first CoAP endpoint from the
   * list of registered endpoints is returned
   */
  private Endpoint getEndpointForClient(Registration registration) {
    ...
    throw new IllegalStateException(
        "can't find the client endpoint for address : " + registration
