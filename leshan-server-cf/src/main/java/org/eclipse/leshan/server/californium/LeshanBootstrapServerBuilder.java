...
/**
 * Class helping you to build and configure a Californium based Leshan Bootstrap Lightweight M2M
 * server. Usage: create it, call the different setters for changing the configuration and then call
 * the {@link #build()} method for creating the {@link LwM2mBootstrapServer} ready to operate.
 */
public class LeshanBootstrapServerBuilder {
  ...
  public LeshanBootstrapServer build() {
    ...
    if (!noSecuredEndpoint) {
      ...
      } else if (localAddressSecure != null && !localAddressSecure
    ...
    return new LeshanBootstrapServer(unsecuredEndpoint, securedEndpoint, configStore, securityStore,
        sessionManager,
        model, coapConfig);
  }
}
