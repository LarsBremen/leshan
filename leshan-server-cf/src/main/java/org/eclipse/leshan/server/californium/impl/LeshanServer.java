 ...
 * <p>
 * This implementation starts a Californium {@link CoapServer} with a non-secure and secure
 * endpoint. This CoAP server defines a <i>/rd</i> resource as described in the CoRE RD
 * specification.
 * </p>
 * <p>
 * This class is the entry point to send synchronous and asynchronous requests to registered
 * clients.
 * </p>
 * <p>
 * The {@link LeshanServerBuilder} should be the preferred way to build an instance of {@link
 * LeshanServer}.
 * </p>
 */
public class LeshanServer implements LwM2mServer {
   ...
   * @param registrationIdProvider to provide registrationId using for location-path option values
   * on response of Register operation.
   */
  public LeshanServer(CoapEndpoint unsecuredEndpoint, CoapEndpoint securedEndpoint,
    ...
    if (noQueueMode) {
      ...
      requestSender = new CaliforniumLwM2mRequestSender(endpoints, observationService,
          modelProvider, encoder,
          decoder);
      ...
      requestSender = new CaliforniumQueueModeRequestSender(presenceService,
          new CaliforniumLwM2mRequestSender(endpoints, observationService, modelProvider, encoder,
              decoder));
    }
  ...
  public class CoapAPI {
    ...
    /**
     * Sends a CoAP request synchronously to a registered LWM2M device. Will block until a response
     * is received from the remote client.
     *
     ...
     * @throws ClientSleepingException if the client is sleeping and then the request cannot be
     * sent. This exception will never be raised if Queue Mode support is deactivate.
     */
    public Response send(Registration destination, Request request) throws InterruptedException {
      ...
      if (!(requestSender instanceof CoapRequestSender)) {
        throw new UnsupportedOperationException(
            "This sender does not support to send CoAP request");
      }
     ...
     * @return the response or <code>null</code> if the timeout expires (given parameter or CoAP
     * timeout).
     * @throws CodecException if request payload can not be encoded.
     * @throws InterruptedException if the thread was interrupted.
     * @throws RequestRejectedException if the request is rejected by foreign peer.
     ...
     * sent. This exception will never be raised if Queue Mode support is deactivate.
     */
    public Response send(Registration destination, Request request, long timeout)
        throws InterruptedException {
      // Ensure that delegated sender is able to send CoAP request
      if (!(requestSender instanceof CoapRequestSender)) {
        throw new UnsupportedOperationException(
     ...
     * @param destination the remote client
     * @param request the request to send to the client
     * @param responseCallback a callback called when a response is received (successful or error
     * response)
     * @param errorCallback a callback called when an error or exception occurred when response is
     * received. It can be :
     * <ul>
     ...
     * sent. This exception will never be raised if Queue Mode support is deactivate.
     */
    public void send(Registration destination, Request request,
        CoapResponseCallback responseCallback,
      ...
      sender
          .sendCoapRequest(destination, request, DEFAULT_TIMEOUT, responseCallback, errorCallback);
    }
     ...
     * sent. This exception will never be raised if Queue Mode support is deactivate.
     */
    public void send(Registration destination, Request request, long timeout,
        CoapResponseCallback responseCallback,
        ErrorCallback errorCallback) {
      // Ensure that delegated sender is able to send CoAP request
      if (!(requestSender instanceof CoapRequestSender)) {
